...
function Navbar() {
  ...
  async function uploadToBlockchain() {
    await shareFile(state.database.fileUrl, state.database.receiverWallet);
    ...
    const tx = await historyContract.pushToHistory(
      state.database.receiverWallet,
      state.database.fileUrl,
  ...
  const progressCallback = (progressData) => {
    ...
    setPct(percentageDone);
    setUploading(true);
  };

  const storeWithLighthouse = async (e) => {
    /*
    ...
    */
    ...
    const response = await lighthouse.uploadEncrypted(
      e,
      sig.publicKey,
      process.env.REACT_APP_LIGHTHOUSE_KEY,
      sig.signedMessage,
      progressCallback
    );
    ...
    /*
	   ...
	   */
    console.log("File Upload Hash : ", response.data.Hash);

    state.setDatabase({
      ...state.database,
      fileUrl: response.data.Hash,
  ...
  const shareFile = async (cid, receiverAddress) => {
    ...
    const publicKeyUserB = [receiverAddress];
    ...
    console.log("Share File Result : ", res);
    /*
      data: {
